  ...
  actions: {
    ...
    async sendMsgExec({ rootGetters }, { value, fee = [], memo = "" }) {
      try {
        ...
        if (e == MissingWalletError) {
          throw new SpVuexError(
            "TxClient:MsgExec:Init",
            "Could not initialize signing client. Wallet is required."
          );
        } else {
          throw new SpVuexError(
            "TxClient:MsgExec:Send",
            "Could not broadcast Tx: " + e.message
          );
        }
      }
    ...
    async sendMsgRevoke({ rootGetters }, { value, fee = [], memo = "" }) {
      try {
        ...
        if (e == MissingWalletError) {
          throw new SpVuexError(
            "TxClient:MsgRevoke:Init",
            "Could not initialize signing client. Wallet is required."
          );
        } else {
          throw new SpVuexError(
            "TxClient:MsgRevoke:Send",
            "Could not broadcast Tx: " + e.message
          );
        }
      }
