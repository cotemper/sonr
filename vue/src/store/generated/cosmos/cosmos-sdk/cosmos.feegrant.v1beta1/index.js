  ...
  actions: {
    ...
    async QueryAllowance(
      ...
      try {
        ...
        let value = (await queryClient.queryAllowance(key.granter, key.grantee))
          .data;
        commit("QUERY", {
          query: "Allowance",
    ...
    async QueryAllowances(
      ...
      try {
        const key = params ?? {};
        const queryClient = await initQueryClient(rootGetters);
        let value = (await queryClient.queryAllowances(key.grantee, query))
          .data;
        while (all && value.pagination && value.pagination.next_key != null) {
          let next_values = (
            await queryClient.queryAllowances(key.grantee, {
              ...query,
    ...
    async sendMsgGrantAllowance(
      ...
      try {
        ...
        if (e == MissingWalletError) {
          throw new SpVuexError(
            "TxClient:MsgGrantAllowance:Init",
            "Could not initialize signing client. Wallet is required."
          );
        } else {
          throw new SpVuexError(
            "TxClient:MsgGrantAllowance:Send",
    ...
    async sendMsgRevokeAllowance(
      ...
      try {
        ...
        if (e == MissingWalletError) {
          throw new SpVuexError(
            "TxClient:MsgRevokeAllowance:Init",
            "Could not initialize signing client. Wallet is required."
          );
        } else {
          throw new SpVuexError(
            "TxClient:MsgRevokeAllowance:Send",
            "Could not broadcast Tx: " + e.message
          );
        }
      }
    ...
    async MsgGrantAllowance({ rootGetters }, { value }) {
      try {
        ...
        if (e == MissingWalletError) {
          ...
          throw new SpVuexError(
            "TxClient:MsgGrantAllowance:Create",
    ...
    async MsgRevokeAllowance({ rootGetters }, { value }) {
      try {
        ...
        if (e == MissingWalletError) {
          ...
          throw new SpVuexError(
            "TxClient:MsgRevokeAllowance:Create",
            "Could not create message: " + e.message
          );
        }
      }
